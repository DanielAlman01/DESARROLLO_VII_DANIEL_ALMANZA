...
function generarReporteBiblioteca($biblioteca) {
    ...
    $reporte['libros_por_genero'] = array_reduce($biblioteca, function($carry, $libro) {
        $genero = $libro['genero'];
        if (!isset($carry[$genero])) {
            $carry[$genero] = 0;
        }
        $carry[$genero]++;
        return $carry;
    ...
    $autorConMasLibros = array_search(max($conteoAutores), $conteoAutores);
    $reporte['autor_con_mas_libros'] = $autorConMasLibros;
